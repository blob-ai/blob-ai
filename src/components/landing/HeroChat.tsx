...
export const HeroChat = () => {
  ...
  // Initialize chat with AI greeting if no thread is selected
  useEffect(() => {
    if (!currentThread) {
      const initChat = async () => {
        ...
        setLocalMessages([{ id: messageId, content: "", type: "ai" }]);
      ...
      // Clear local messages when thread is selected and show chat messages
      setLocalMessages([]);
      setActionButtons([]);
      setSuggestionChips([]);
    }
  ...
  // Handle thread selection and message loading
  useEffect(() => {
    if (currentThread) {
  ...
  // Get messages to display (either local demo messages or real chat messages)
  const messagesToDisplay = currentThread ? chatMessages : localMessages;
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center p-8 text-center">
      <div className="w-12 h-12 rounded-full bg-primary-900/50 flex items-center justify-center text-primary-400 mb-4">
      ...
      <p className="text-white/60 text-sm">Select a chat thread from the sidebar or create a new one to begin chatting.</p>
    </div>
  );
  ...
  const LoadingState = () => (
    <div className="flex flex-col items-center justify-center p-8">
      ...
      <p className="text-white/60 text-sm mt-2">Loading messages...</p>
    </div>
  );

  // Render messages based on current state
  const renderMessages = () => {
    if (currentThread && loadingMessages) {
      return <LoadingState />;
    }
    
    if (currentThread && chatMessages.length === 0) {
      return <EmptyState />;
    }
    
    if (!currentThread && localMessages.length === 0) {
      return null; // Will show initial typing animation
    }

    const messages = currentThread ? chatMessages : localMessages;
    ...
    return messages
      ...
      .map((message) => (
        <div 
          key={message.id} 
          className={`flex ${(currentThread ? message.sender === 'user' : message.type === 'user') ? 'justify-end' : 'justify-start'}`}
        >
          {(currentThread ? message.sender === 'assistant' : message.type === 'ai') && (
          ...
          <div 
            className={`chat-message ${
              (currentThread ? message.sender === 'user' : message.type === 'user')
                ? 'chat-message-user' 
                : 'chat-message-ai'
            }`}
          >
            {(currentThread ? message.text : message.content) ? (
              <div dangerouslySetInnerHTML={{ __html: currentThread ? message.text : message.content }} />
            ) : (
    ...
    <div className="chat-container w-[80%] mx-auto ai-message-glow">
      <div className="chat-header">
        ...
        <div className="text-sm text-white/60">
          {currentThread ? currentThread.title : "inspire.me chat"}
        </div>
        <div className="w-8"></div>
      </div>
      ...
      <div className="p-5 min-h-[280px] flex flex-col" ref={chatContainerRef}>
        <div className="flex-1 flex flex-col space-y-4 overflow-y-auto">
          {currentThread && chatMessages.length === 0 && !loadingMessages ? (
            <EmptyState />
          ) : currentThread && loadingMessages ? (
            <LoadingState />
          ) : (
            renderMessages()
          )}
